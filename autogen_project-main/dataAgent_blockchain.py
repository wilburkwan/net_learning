...
async def process_chunk(chunk, start_idx, total_records, model_client, termination_condition):
    ...
    # 說明代理人需使用繁體中文回應
    language_instruction = "請用繁體中文回應以下問題。\n"

    prompt = (
        f"{language_instruction}\n"
        f"目前正在處理第 {start_idx} 至 {start_idx + len(chunk) - 1} 筆區塊鏈交易資料（共 {total_records} 筆）。\n"
        f"以下為該批次資料:\n{chunk_data}\n\n"
        "請根據以上資料進行分析，並提供完整的區塊鏈交易建議。"
        "其中請特別注意：\n"
        "  1. 分析交易中的參與者、交易額、時間戳記等要素；\n"
        "  2. 請 MultimodalWebSurfer 搜尋外部網站，找出最新的區塊鏈技術和交易安全建議（例如防範交易詐騙、提升交易效率等），\n"
        "     並將搜尋結果整合進回覆中；\n"
        "  3. 最後請提供具體的建議和相關參考資訊，特別是在區塊鏈交易方面的風險管理。\n"
...
async def main():
    ...
    csv_file_path = "suspicious_node.csv"  # 替換為區塊鏈資料的 CSV 檔案
    chunk_size = 1000
